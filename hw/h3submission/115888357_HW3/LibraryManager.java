...
public class LibraryManager {
    ...
    private static void handleCheckoutBook() {
        try {
            ...
            if(bookRepository.checkDoesNotExist(Util.convertISBNToLong(isbn))) {
            ...
            Book book = bookRepository.fetch(Util.convertISBNToLong(isbn));
            String fmt = "%s has been checked out by %s and must be returned on %s\n";
            System.out.format(fmt, book.getName(), id, dueDate.toString());
        } catch(Exception e) {
    ...
    private static void handleSortShelf() {
        try {
            ...
            System.out.println("Successfully sorted shelf " + shelf + " by " + SortCriteria.parseString(sc));
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
    }
